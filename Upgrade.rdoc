...
anyway!), which was supposed to treat be_true as equal(true), and be_false
as equal(false). 

If the actual values +true+ and +false+ are meaningful to your examples,
you'll want to change the to use equal(true) and equal(false) (or ==(true)
and ==(false)).
