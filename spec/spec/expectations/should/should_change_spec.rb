...
class SomethingExpected
  ...
  def reset!
    self.some_value = rand(1000)
  end
end

context "should_change" do
  setup do
...
context "should_change.from(x).to(y)" do
  setup do
    @instance = SomethingExpected.new
    @instance.some_value = 'string'
  end
