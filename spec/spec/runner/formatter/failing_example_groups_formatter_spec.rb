    ...
    module Formatter
      describe FailingExampleGroupsFormatter do
        ...
        it "should add example name for each failure" do
          formatter.add_example_group(Class.new(::Spec::Example::ExampleGroupDouble).describe("b 1").report)
          formatter.example_failed("e 1", nil, Spec::Runner::Reporter::Failure.new("g", nil, RuntimeError.new))
          formatter.add_example_group(Class.new(::Spec::Example::ExampleGroupDouble).describe("b 2").report)
        ...
        it "should remove druby url, which is used by Spec::Distributed" do
          @formatter.add_example_group(Class.new(::Spec::Example::ExampleGroupDouble).describe("something something (druby://99.99.99.99:99)").report)
          @formatter.example_failed("e 1", nil, ::Spec::Runner::Reporter::Failure.new("g", nil, RuntimeError.new))
