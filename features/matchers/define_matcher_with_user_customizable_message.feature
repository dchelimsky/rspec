Feature: define matcher with user customizable failure message
  ...
  Scenario: one additional method
    Given a file named "node_spec.rb.rb" with:
      ...
      describe "a new Node" do
        it "should be waiting" do
          ...
          node.should be_waiting, :or => "node.state: #{node.state} (first example)"
        end
        
        it "should not be started" do
          node = Node.new(:started) #start w/ started to trigger failure
          node.should_not be_started, :or => "node.state: #{node.state} (second example)"
        end
      end
      ...
      describe "node.start" do
        it "should change the state" do
          node = Node.new(:started) #start w/ started to trigger failure
          lambda {node.start}.should change{node.state}, :or => "expected a change"
        end
      end
    ...
    And  the stdout should match "expected waiting? to return true, got false"
    And  the stdout should match "expected started? to return false, got true"
