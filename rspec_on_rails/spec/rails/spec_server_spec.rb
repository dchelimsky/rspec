...
if RUBY_PLATFORM =~ /darwin/
  # This stops the entire spec from running on mac. There had been a pending
  # statement in before(:each), but pending doesn't prevent after(:each) from
  # trying to kill processes that were never created or, scarier,
  # exec 'rm /' when "#{tmbundle_install_directory}/#{@bundle_name}"
  # resolves to "/".
  
  describe "script/spec_server" do
    # TODO: Please fix this if it does not work on your machine. Otherwise there will be NO coverage of spec_server.
    it "should NOT be hosed on the mac" do
      pending("spec server is intermittently hanging on the mac platform")
    end
