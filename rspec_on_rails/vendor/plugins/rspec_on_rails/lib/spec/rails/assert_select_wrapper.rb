###################################################################################################
# This is a very slightly tweaked port of assert_select for RSpec on Rails. The rdocs all still
# look like assert_select - so 
  ...
  module Rails #:nodoc:
    ...
    class AssertSelectWrapper
      ...
      def should_have(*args, &block)
        ...
        if match_with = equals[:text]
          matches.delete_if do |match|
            ...
            unless match_with.is_a?(Regexp) ? (text =~ match_with) : (text == match_with.to_s)
              content_mismatch ||= "expected <#{match_with}> but got <#{text}>"
            end
          end
          ...
          matches.delete_if do |match|
            ...
            unless match_with.is_a?(Regexp) ? (html =~ match_with) : (html == match_with.to_s)
              content_mismatch ||= "expected <#{match_with}> but got <#{html}>"
              true
            end
          end
        end
        ...
        if equals[:minimum]
          unless matches.size >= equals[:minimum]
            fail_with("Expected at least #{equals[:minimum]} elements, found #{matches.size}.")
          end
        end
        if equals[:maximum]
          unless matches.size <= equals[:maximum]
            fail_with("Expected at most #{equals[:maximum]} elements, found #{matches.size}.")
          end
        end
      ...
      def should_have_feed(type, version = nil, &block)
        ...
        response.should_have root, selector, &block
      end
      ...
      def should_have_encoded(element = nil, &block)
        ...
        begin
          ...
          response.should_have ":root", &block
        ensure
      ...
      def should_have_email(&block)
        ...
        if deliveries.empty?
          fail_with("No e-mail in delivery list")
        end
        for delivery in deliveries
          root = HTML::Document.new(delivery.body).root
          response.should_have root, ":root", &block
        end
      end
