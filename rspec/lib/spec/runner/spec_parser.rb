    ...
    protected
      ...
      def parse_description(str)
        ...
        if matches = /^['"](.*)['"](,.*)?$/.match(str)
          return ::Spec::DSL::ExampleGroupDescription.generate_description(matches[1])
        end
        if matches = /^(.*)\s*,\s*['"](.*)['"](,.*)?$/.match(str)
          return ::Spec::DSL::ExampleGroupDescription.generate_description(matches[1], matches[2])
        end
        return str
      end
    end
